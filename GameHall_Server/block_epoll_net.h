...
struct SendTask {
    SendTask(){}
    SendTask(sock_fd sockfd, char* buf, int nlen):
        m_sockfd(sockfd), m_buf(buf), m_nlen(nlen){}
    sock_fd m_sockfd;
    char* m_buf;
    int m_nlen;
};
...
public:
    ...
    void EventLoop();
    //发送数据
    void SendData(int fd, char* szbuf , int nlen );
...
private:
    ...
    pool_t *m_pool;
    //读写队列
    static MyQueue<SendTask*> SendQue;
